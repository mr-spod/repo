...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        // if the size of the tree is greater than 1
        if (array.length() > 1) {
        	// assigning indexes to variables
        		int leftIndex = (top * 2) + 1;
        		int rightIndex = (top * 2) + 2;
        		if (leftIndex <= last) {
        			// assigns entry of leftIndex to leftTop
            		T leftTop = array.entry(leftIndex);
            		// assigns leftIndex to smallerIndex
            		int smallerIndex = leftIndex;
        			if (rightIndex <= last) {
        				// assigns entry of rightIndex to rightTop
                		T rightTop = array.entry(rightIndex);
                		if (order.compare(rightTop, leftTop) < 0) {
                			// assigns rightIndex to smallerIndex
                			// if rightTop is less than leftTop
                			smallerIndex = rightIndex;
                		}
        			}
        			if (order.compare(array.entry(top), array.entry(smallerIndex)) >= 0) {
        				// exchange entries of smallerIndex and top
                		array.exchangeEntries(smallerIndex, top);
                		// calls recursion on subtree from root smallerIndex
                		siftDown(array, smallerIndex, last, order);
        			}
        		}
        }
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        if (array.length() > 1) {
        		// Indices for the roots of the left and right subtrees
            int left = top * 2 + 1;
            int right = top * 2 + 2;
            // Index for last element of array
            ...
            // Sift down the element at top to make the subtree from top down a heap
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        Array<T> a = new Array1L<T>(q.length());
        int i = 0;
        while (q.length() > 0) {
        		a.setEntry(i, q.dequeue());
        		i++;
        }
        heapify(a, 0, order);
    ...
    public final T removeFirst() {
        ...
        // Replace the last element in the heap with the root
        T bottom = this.heap.replaceEntry(this.heapSize - 1, first);
        // Reduce the size of the heap by 1
        this.heapSize--;
        // Place the entry that was last in the heap on the top of the heap
        this.heap.setEntry(0, bottom);
        // Sift down the new root of the tree (at this point we know the two subtrees are heaps)
        siftDown(this.heap, 0, this.heapSize - 1, this.machineOrder);

        assert this.conventionHolds();

        // Return the initial root of the heap
        return first;
    }
