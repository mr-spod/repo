...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        // Runs a for loop to set the entry at each index as a new Map
    ...
    public Map4() {
        ...
        // Initial size is 0 (no K/V pairs yet)
        this.size = 0;

        // Creates a hashtable with the default hashtable size.
    ...
    public Map4(int hashTableSize) {
        ...
        // Initial size is 0 (no K/V pairs yet)
        this.size = 0;
        // Create a new hashtable with the give hashTableSize
    ...
    public final void add(K key, V value) {
        ...
        // hashTable index for this key
        int i = mod(key.hashCode(), this.hashTable.length());
        // puts the Map to the index's corresponding bucket
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // hashTable index for this key
        int i = mod(key.hashCode(), this.hashTable.length());
        // Variable for the map containing the given key
        Map<K, V> m = this.hashTable.entry(i);
        // Decreases the size of this by 1
        this.size--;
        // Returns the Pair with key "key", removing it from the map
        return m.remove(key);
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        for (Map<K, V> map : this.hashTable) {
            // if the size of this map is larger than 0, assign it to m variable
            if (map.size() > 0) {
                m = map;
            }
        }
        // Decreases the size of this by 1
        this.size--;
        // Removes any pair from this map (bucket w/ size > 0) and retruns it
        return m.removeAny();
    }
    ...
    public final V value(K key) {
        ...
        // hashTable index for the given key
        int i = mod(key.hashCode(), this.hashTable.length());
        // Returns the value of the key in the map at entry i in the hashTable
    ...
    public final boolean hasKey(K key) {
        ...
        if (this.hashTable.mayBeExamined(i)) {
            // returns a boolean value whether if the map at index i of the hashTable contains the given key
