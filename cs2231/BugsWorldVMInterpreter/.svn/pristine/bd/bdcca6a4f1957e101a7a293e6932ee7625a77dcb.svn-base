...
public final class BugsWorldVMInterpreter {
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs,
        ...
        int nextByte = cp[pc];
        while (!isPrimitiveInstructionByteCode(nextByte)) {
        		if (nextByte == Instruction.JUMP.byteCode()) {
        			int jumpPosition = cp[pc+1];
        			pc = jumpPosition;
        		} else {
        			boolean jumpCondition = conditionalJumpCondition(wbs, nextByte);
            		if (!jumpCondition) {
            			int jumpPosition = cp[pc+1];
            			pc = jumpPosition;
            		} else {
